...
def get_video_list(cmsUrl, pagecount):
    global allcount
    with tqdm(total=int(allcount)) as pbar:
        pbar.set_description('当前影片数量：')
        with ThreadPoolExecutor(50) as t:        
            for pg in range(1, int(pagecount) + 1):
                t.submit(get_video_and_insert, cmsUrl, pg, pbar)

def get_video_and_insert(cmsUrl, pg, pbar):
    try:
...
def get_all_video(cmsUrl):
    pagecount = get_start_info(cmsUrl)
    get_video_list(cmsUrl, pagecount)