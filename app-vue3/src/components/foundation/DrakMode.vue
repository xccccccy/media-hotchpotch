<template>
  <div :class="{ headeractivate: dark_mode_span_show }" class="relative" v-clickOutSide="clickNoDom">
    <div @click="dark_mode_span_show = !dark_mode_span_show">
      <Awake class="w-6 h-6 dark:hidden"></Awake>
      <Asleep class="w-6 h-6 hidden dark:block"></Asleep>
    </div>
...
import $ from "jquery";
import Awake from '~icons/ri/sun-line'
import Asleep from '~icons/ri/moon-line'

export default {
  name: 'DarkModeComponent',
  emits: ["darkModeChange"],
  components: { Awake, Asleep },
  setup(props, context) {
    const darkmode = ref('no');
    const dark_mode_span_show = ref(false);
    ...
    let callback = e => {
      ...
      context.emit('darkModeChange');
    }

    watch(darkmode, (newmode, oldValue) => {
      if (newmode == 'dark') {
        $('html').addClass('dark');
